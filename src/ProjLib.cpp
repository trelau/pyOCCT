...
PYBIND11_MODULE(ProjLib, mod) {
	...
	py::class_<ProjLib_ProjectedCurve, std::unique_ptr<ProjLib_ProjectedCurve, Deleter<ProjLib_ProjectedCurve>>, Adaptor2d_Curve2d> cls_ProjLib_ProjectedCurve(mod, "ProjLib_ProjectedCurve", "Compute the 2d-curve. Try to solve the particular case if possible. Otherwise, an approximation is done.");
	...
	cls_ProjLib_PrjResolve.def("Perform", (void (ProjLib_PrjResolve::*)(const Standard_Real, const Standard_Real, const Standard_Real, const gp_Pnt2d &, const gp_Pnt2d &, const gp_Pnt2d &, const Standard_Real, const Standard_Boolean)) &ProjLib_PrjResolve::Perform, "Calculates the ort from C(t) to S with a close point. The close point is defined by the parameter values U0 and V0. The function F(u,v)=distance(S(u,v),C(t)) has an extremum when gradient(F)=0. The algorithm searches a zero near the close point.", py::arg("t"), py::arg("U"), py::arg("V"), py::arg("Tol"), py::arg("Inf"), py::arg("Sup"), py::arg("FTol"), py::arg("StrictInside"));
