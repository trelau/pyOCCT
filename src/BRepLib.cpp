...
PYBIND11_MODULE(BRepLib, mod) {
	...
	cls_BRepLib_MakePolygon.def("Added", (Standard_Boolean (BRepLib_MakePolygon::*)() const ) &BRepLib_MakePolygon::Added, "Returns True if the last vertex or point was successfully added.");
	...
	py::class_<BRepLib_MakeShell, std::unique_ptr<BRepLib_MakeShell, Deleter<BRepLib_MakeShell>>, BRepLib_MakeShape> cls_BRepLib_MakeShell(mod, "BRepLib_MakeShell", "Provides methods to build shells.");
	...
	cls_BRepLib_FindSurface.def("Init", (void (BRepLib_FindSurface::*)(const TopoDS_Shape &, const Standard_Real, const Standard_Boolean, const Standard_Boolean)) &BRepLib_FindSurface::Init, "Computes the Surface from the edges of <S> with the given tolerance. if <OnlyPlane> is true, the computed surface will be a plane. If it is not possible to find a plane, the flag NotDone will be set. If <OnlyClosed> is true, then S should be a wire and the existing surface, on which wire S is not closed in 2D, will be ignored.", py::arg("S"), py::arg("Tol"), py::arg("OnlyPlane"), py::arg("OnlyClosed"));
	...
	cls_BRepLib.def_static("EncodeRegularity_", (void (*)(TopoDS_Edge &, const TopoDS_Face &, const TopoDS_Face &, const Standard_Real)) &BRepLib::EncodeRegularity, "Encodes the Regularity between <F1> and <F2> by <E> Warning: <TolAng> is an angular tolerance, expressed in Rad. Warning: If the edge's regularity is coded before, nothing is done.", py::arg("E"), py::arg("F1"), py::arg("F2"), py::arg("TolAng"));
	...
	cls_BRepLib.def_static("BoundingVertex_", [](const NCollection_List<TopoDS_Shape> & theLV, gp_Pnt & theNewCenter, Standard_Real & theNewTol){ BRepLib::BoundingVertex(theLV, theNewCenter, theNewTol); return theNewTol; }, "Calculates the bounding sphere around the set of vertices from the theLV list. Returns the center (theNewCenter) and the radius (theNewTol) of this sphere. This can be used to construct the new vertex which covers the given set of other vertices.", py::arg("theLV"), py::arg("theNewCenter"), py::arg("theNewTol"));
