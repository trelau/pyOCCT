...
PYBIND11_MODULE(CSLib, mod) {
	...
	cls_CSLib_NormalPolyDef.def("Value", (Standard_Boolean (CSLib_NormalPolyDef::*)(const Standard_Real, Standard_Real &)) &CSLib_NormalPolyDef::Value, "computes the value <F>of the function for the variable <X>. Returns True if the calculation were successfully done, False otherwise.", py::arg("X"), py::arg("F"));
	cls_CSLib_NormalPolyDef.def("Derivative", (Standard_Boolean (CSLib_NormalPolyDef::*)(const Standard_Real, Standard_Real &)) &CSLib_NormalPolyDef::Derivative, "computes the derivative <D> of the function for the variable <X>. Returns True if the calculation were successfully done, False otherwise.", py::arg("X"), py::arg("D"));
	cls_CSLib_NormalPolyDef.def("Values", (Standard_Boolean (CSLib_NormalPolyDef::*)(const Standard_Real, Standard_Real &, Standard_Real &)) &CSLib_NormalPolyDef::Values, "computes the value <F> and the derivative <D> of the function for the variable <X>. Returns True if the calculation were successfully done, False otherwise.", py::arg("X"), py::arg("F"), py::arg("D"));
	...
	cls_CSLib.def_static("Normal_", (void (*)(const Standard_Integer, const TColgp_Array2OfVec &, const Standard_Real, const Standard_Real, const Standard_Real, const Standard_Real, const Standard_Real, const Standard_Real, const Standard_Real, CSLib_NormalStatus &, gp_Dir &, Standard_Integer &, Standard_Integer &)) &CSLib::Normal, "find the first order k0 of deriviative of NUV where: foreach order < k0 all the derivatives of NUV are null all the derivatives of NUV corresponding to the order k0 are collinear and have the same sens. In this case, normal at U,V is unique.", py::arg("MaxOrder"), py::arg("DerNUV"), py::arg("MagTol"), py::arg("U"), py::arg("V"), py::arg("Umin"), py::arg("Umax"), py::arg("Vmin"), py::arg("Vmax"), py::arg("theStatus"), py::arg("Normal"), py::arg("OrderU"), py::arg("OrderV"));
