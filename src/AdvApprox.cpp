...
PYBIND11_MODULE(AdvApprox, mod) {
	...
	cls_AdvApprox_PrefAndRec.def("Value", (Standard_Boolean (AdvApprox_PrefAndRec::*)(const Standard_Real, const Standard_Real, Standard_Real &) const ) &AdvApprox_PrefAndRec::Value, "cuting value is - the recommended point nerest of (a+b)/2 if pi is in ]a,b[ or else - the preferential point nearest of (a+b) / 2 if pi is in ](r*a+b)/(r+1) , (a+r*b)/(r+1)[ where r = Weight - or (a+b)/2 else.", py::arg("a"), py::arg("b"), py::arg("cuttingvalue"));
