...
PYBIND11_MODULE(AdvApprox, mod) {
	...
	cls_AdvApprox_Cutting.def("Value", (Standard_Boolean (AdvApprox_Cutting::*)(const Standard_Real, const Standard_Real, Standard_Real &) const ) &AdvApprox_Cutting::Value, "None", py::arg("a"), py::arg("b"), py::arg("cuttingvalue"));
	...
	// FIXME cls_AdvApprox_EvaluatorFunction.def("Evaluate", (void (AdvApprox_EvaluatorFunction::*)(Standard_Integer *, Standard_Real [2], Standard_Real *, Standard_Integer *, Standard_Real *, Standard_Integer *)) &AdvApprox_EvaluatorFunction::Evaluate, "Function evaluation method to be defined by descendant", py::arg("Dimension"), py::arg("StartEnd"), py::arg("Parameter"), py::arg("DerivativeRequest"), py::arg("Result"), py::arg("ErrorCode"));
	...
	cls_AdvApprox_ApproxAFunction.def_static("Approximation_", (void (*)(const Standard_Integer, const Standard_Integer, const TColStd_Array1OfInteger &, const Standard_Real, const Standard_Real, AdvApprox_EvaluatorFunction &, const AdvApprox_Cutting &, const Standard_Integer, const Standard_Integer, const Standard_Integer, const TColStd_Array1OfReal &, const Standard_Integer, Standard_Integer &, TColStd_Array1OfInteger &, TColStd_Array1OfReal &, TColStd_Array1OfReal &, TColStd_Array1OfReal &, TColStd_Array1OfReal &, Standard_Integer &)) &AdvApprox_ApproxAFunction::Approximation, "None", py::arg("TotalDimension"), py::arg("TotalNumSS"), py::arg("LocalDimension"), py::arg("First"), py::arg("Last"), py::arg("Evaluator"), py::arg("CutTool"), py::arg("ContinuityOrder"), py::arg("NumMaxCoeffs"), py::arg("MaxSegments"), py::arg("TolerancesArray"), py::arg("code_precis"), py::arg("NumCurves"), py::arg("NumCoeffPerCurveArray"), py::arg("LocalCoefficientArray"), py::arg("IntervalsArray"), py::arg("ErrorMaxArray"), py::arg("AverageErrorArray"), py::arg("ErrorCode"));
	...
	cls_AdvApprox_DichoCutting.def("Value", (Standard_Boolean (AdvApprox_DichoCutting::*)(const Standard_Real, const Standard_Real, Standard_Real &) const ) &AdvApprox_DichoCutting::Value, "None", py::arg("a"), py::arg("b"), py::arg("cuttingvalue"));
	...
	// FIXME cls_AdvApprox_PrefAndRec.def("Value", (Standard_Boolean (AdvApprox_PrefAndRec::*)(const Standard_Real, const Standard_Real, Standard_Real &) const ) &AdvApprox_PrefAndRec::Value, "cuting value is - the recommended point nerest of (a+b)/2 if pi is in ]a,b[ or else - the preferential point nearest of (a+b) / 2 if pi is in ](r*a+b)/(r+1) , (a+r*b)/(r+1)[ where r = Weight - or (a+b)/2 else.", py::arg("a"), py::arg("b"), py::arg("cuttingvalue"));
	...
	cls_AdvApprox_PrefCutting.def("Value", (Standard_Boolean (AdvApprox_PrefCutting::*)(const Standard_Real, const Standard_Real, Standard_Real &) const ) &AdvApprox_PrefCutting::Value, "None", py::arg("a"), py::arg("b"), py::arg("cuttingvalue"));
