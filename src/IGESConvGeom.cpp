...
PYBIND11_MODULE(IGESConvGeom, mod) {
	...
	cls_IGESConvGeom_GeomBuilder.def("EvalXYZ", (void (IGESConvGeom_GeomBuilder::*)(const gp_XYZ &, Standard_Real &, Standard_Real &, Standard_Real &) const ) &IGESConvGeom_GeomBuilder::EvalXYZ, "Evaluates a XYZ value in the Position already defined. Returns the transformed coordinates. For a 2D definition, X,Y will then be used to define a XY and Z will be regarded as a Z Displacement (can be ignored)", py::arg("val"), py::arg("X"), py::arg("Y"), py::arg("Z"));
