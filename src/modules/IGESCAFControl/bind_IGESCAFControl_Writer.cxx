...
py::class_<IGESCAFControl_Writer, std::unique_ptr<IGESCAFControl_Writer>, IGESControl_Writer> cls_IGESCAFControl_Writer(mod, "IGESCAFControl_Writer", "Provides a tool to write DECAF document to the IGES file. Besides transfer of shapes (including assemblies) provided by IGESControl, supports also colors and part names IGESCAFControl_Writer writer(); Methods for writing IGES file: writer.Transfer (Document); writer.Write('filename') or writer.Write(OStream) or writer.Perform(Document,'filename'); Methods for managing the writing of attributes. Colors writer.SetColorMode(colormode); Standard_Boolean colormode = writer.GetColorMode(); Layers writer.SetLayerMode(layermode); Standard_Boolean layermode = writer.GetLayerMode(); Names writer.SetNameMode(namemode); Standard_Boolean namemode = writer.GetNameMode();");
