...
PYBIND11_MODULE(VrmlData, mod) {
	...
	// FIXME cls_VrmlData_Scene.def("bits_left", (VrmlData_Scene & (VrmlData_Scene::*)(Standard_IStream &)) &VrmlData_Scene::operator<<, py::is_operator(), "Import from text stream (file or else). This method is protected by Mutex, it is not allowed to read/write two VRML streams concurrently.", py::arg("theInput"));
