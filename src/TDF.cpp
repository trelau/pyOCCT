...
PYBIND11_MODULE(TDF, mod) {
	...
	*/
	...
	cls_TDF_Attribute.def("ForgetAttribute", (Standard_Boolean (TDF_Attribute::*)(const Standard_GUID &) const ) &TDF_Attribute::ForgetAttribute, "Forgets the Attribute of GUID <aguid> associated to the label of <me>. Be careful that if <me> is the attribute of <guid>, <me> will have a null label after this call. If the attribute doesn't exist returns False. Otherwise returns True.", py::arg("aguid"));
	cls_TDF_Attribute.def("ForgetAllAttributes", [](TDF_Attribute &self) -> void { return self.ForgetAllAttributes(); });
	cls_TDF_Attribute.def("ForgetAllAttributes", (void (TDF_Attribute::*)(const Standard_Boolean) const ) &TDF_Attribute::ForgetAllAttributes, "Forgets all the attributes attached to the label of <me>. Does it on the sub-labels if <clearChildren> is set to true. Of course, this method is compatible with Transaction & Delta mecanisms. Be careful that if <me> will have a null label after this call", py::arg("clearChildren"));
