...
PYBIND11_MODULE(OSD, mod) {
	...
	cls_OSD_Chronometer.def("Show", (void (OSD_Chronometer::*)(Standard_Real &) const ) &OSD_Chronometer::Show, "Returns the current CPU user time in a variable. The chronometer can be running (laps Time) or stopped.", py::arg("theUserSeconds"));
	cls_OSD_Chronometer.def("Show", (void (OSD_Chronometer::*)(Standard_Real &, Standard_Real &) const ) &OSD_Chronometer::Show, "Returns the current CPU user and system time in variables. The chronometer can be running (laps Time) or stopped.", py::arg("theUserSeconds"), py::arg("theSystemSeconds"));
	cls_OSD_Chronometer.def_static("GetProcessCPU_", (void (*)(Standard_Real &, Standard_Real &)) &OSD_Chronometer::GetProcessCPU, "Returns CPU time (user and system) consumed by the current process since its start, in seconds. The actual precision of the measurement depends on granularity provided by the system, and is platform-specific.", py::arg("UserSeconds"), py::arg("SystemSeconds"));
	cls_OSD_Chronometer.def_static("GetThreadCPU_", (void (*)(Standard_Real &, Standard_Real &)) &OSD_Chronometer::GetThreadCPU, "Returns CPU time (user and system) consumed by the current thread since its start. Note that this measurement is platform-specific, as threads are implemented and managed differently on different platforms and CPUs.", py::arg("UserSeconds"), py::arg("SystemSeconds"));
	...
	cls_OSD_Timer.def("Show", (void (OSD_Timer::*)(Standard_Real &, Standard_Integer &, Standard_Integer &, Standard_Real &) const ) &OSD_Timer::Show, "returns both the elapsed time(seconds,minutes,hours) and CPU time.", py::arg("theSeconds"), py::arg("theMinutes"), py::arg("theHours"), py::arg("theCPUtime"));
	...
	*/
	...
	cls_OSD.def_static("CStringToReal_", (Standard_Boolean (*)(const Standard_CString, Standard_Real &)) &OSD::CStringToReal, "Converts aCstring representing a real with a period as decimal point, no thousand separator and no grouping of digits into aReal . The conversion is independant from the current locale.", py::arg("aString"), py::arg("aReal"));
