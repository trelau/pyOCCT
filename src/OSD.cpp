...
PYBIND11_MODULE(OSD, mod) {
	...
	cls_OSD_File.def("IsReadable", (Standard_Boolean (OSD_File::*)()) &OSD_File::IsReadable, "returns TRUE if the file exists and if the user has the authorization to read it.");
	...
	cls_OSD_Environment.def("SetValue", (void (OSD_Environment::*)(const TCollection_AsciiString &)) &OSD_Environment::SetValue, "Changes environment variable value. Raises ConstructionError either if the string contains characters not in range of ' '...'~' or if the string contains the character '$' which is forbidden.", py::arg("Value"));
	...
	cls_OSD_Environment.def("SetName", (void (OSD_Environment::*)(const TCollection_AsciiString &)) &OSD_Environment::SetName, "Changes environment variable name. Raises ConstructionError either if the string contains characters not in range of ' '...'~' or if the string contains the character '$' which is forbidden.", py::arg("name"));
	...
	*/
	...
	// C:\Miniconda\envs\occt\Library\include\opencascade\OSD.hxx
	py::class_<OSD, std::unique_ptr<OSD, Deleter<OSD>>> cls_OSD(mod, "OSD", "Set of Operating System Dependent Tools (O)perating (S)ystem (D)ependent");
	cls_OSD.def(py::init<>());
	...
	// FIXME cls_OSD.def_static("RealToCString_", (Standard_Boolean (*)(const Standard_Real, Standard_PCharacter &)) &OSD::RealToCString, "Converts aReal into aCstring in exponential format with a period as decimal point, no thousand separator and no grouping of digits. The conversion is independent from the current locale", py::arg("aReal"), py::arg("aString"));
	cls_OSD.def_static("CStringToReal_", [](const Standard_CString aString, Standard_Real & aReal){ Standard_Boolean rv = OSD::CStringToReal(aString, aReal); return std::tuple<Standard_Boolean, Standard_Real &>(rv, aReal); }, "Converts aCstring representing a real with a period as decimal point, no thousand separator and no grouping of digits into aReal . The conversion is independent from the current locale.", py::arg("aString"), py::arg("aReal"));
