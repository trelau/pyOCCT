...
PYBIND11_MODULE(MoniTool, mod) {
	...
	cls_MoniTool_TypedValue.def("EnumCase", (Standard_Integer (MoniTool_TypedValue::*)(const Standard_CString) const ) &MoniTool_TypedValue::EnumCase, "Returns the case number which cooresponds to a string value Works with main and additional values Returns (StartEnum - 1) if not OK, -1 if not an Enum", py::arg("val"));
	...
	// FIXME 	cls_MoniTool_AttrList.def("GetStringAttribute", [](MoniTool_AttrList &self, const Standard_CString name, Standard_CString & val){ Standard_Boolean rv = self.GetStringAttribute(name, val); return std::tuple<Standard_Boolean, Standard_CString &>(rv, val); }, "Returns an attribute from its name, as String If no attribute has this name, or not a String <val> is 0.0 and returned value is False Else, it is True", py::arg("name"), py::arg("val"));
	...
	// FIXME 	cls_MoniTool_CaseData.def("Text", [](MoniTool_CaseData &self, const Standard_Integer nd, Standard_CString & text){ Standard_Boolean rv = self.Text(nd, text); return std::tuple<Standard_Boolean, Standard_CString &>(rv, text); }, "Returns a text (stored in TCollection_HAsciiString)", py::arg("nd"), py::arg("text"));
	...
	py::class_<MoniTool_DataInfo, std::unique_ptr<MoniTool_DataInfo, Deleter<MoniTool_DataInfo>>> cls_MoniTool_DataInfo(mod, "MoniTool_DataInfo", "Gives information on an object Used as template to instantiate Elem, etc This class is for Transient");
