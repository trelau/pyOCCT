...
PYBIND11_MODULE(TCollection, mod) {
	...
	cls_TCollection_ExtendedString.def("Value", (Standard_ExtCharacter (TCollection_ExtendedString::*)(const Standard_Integer) const ) &TCollection_ExtendedString::Value, "Returns character at position <where> in <me>. If <where> is less than zero or greater than the length of <me>, an exception is raised. Example: aString contains 'Hello' aString.Value(2) returns 'e' Exceptions Standard_OutOfRange if where lies outside the bounds of this extended string.", py::arg("where"));
	...
	cls_TCollection_AsciiString.def("Value", (Standard_Character (TCollection_AsciiString::*)(const Standard_Integer) const ) &TCollection_AsciiString::Value, "Returns character at position <where> in <me>. If <where> is less than zero or greater than the length of <me>, an exception is raised. Example: aString contains 'Hello' aString.Value(2) returns 'e'", py::arg("where"));
