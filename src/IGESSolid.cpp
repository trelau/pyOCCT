...
PYBIND11_MODULE(IGESSolid, mod) {
	...
	cls_IGESSolid_TopoBuilder.def("MakeEdge", (void (IGESSolid_TopoBuilder::*)(const Standard_Integer, const Standard_Integer, const Standard_Integer)) &IGESSolid_TopoBuilder::MakeEdge, "Defines an Edge(UV), to be added in the current Loop by EndEdge <edgetype> gives the type of the edge <edge3d> identifies the Edge(3D) used as support The EdgeList is always the current one <orientation gives the orientation flag It is then necessary to : - give the parametric curves - close the definition of this edge(UV) by EndEdge, else the next call to MakeEdge will erase this one", py::arg("edgetype"), py::arg("edge3d"), py::arg("orientation"));
