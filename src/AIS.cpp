...
PYBIND11_MODULE(AIS, mod) {
	...
	cls_AIS_InteractiveContext.def("Unhilight", (void (AIS_InteractiveContext::*)(const opencascade::handle<AIS_InteractiveObject> &, const Standard_Boolean)) &AIS_InteractiveContext::Unhilight, "Removes highlighting from the Object.", py::arg("theIObj"), py::arg("theToUpdateViewer"));
	...
	cls_AIS_InteractiveContext.def("HilightNextDetected", (Standard_Integer (AIS_InteractiveContext::*)(const opencascade::handle<V3d_View> &, const Standard_Boolean)) &AIS_InteractiveContext::HilightNextDetected, "If more than 1 object is detected by the selector, only the 'best' owner is highlighted at the mouse position. This Method allows the user to highlight one after another the other detected entities. If The method select is called, the selected entity will be the highlighted one! WARNING: Loop Method. When all the detected entities have been highlighted, the next call will highlight the first one again.", py::arg("theView"), py::arg("theToRedrawImmediate"));
	...
	cls_AIS_InteractiveContext.def("Select", (AIS_StatusOfPick (AIS_InteractiveContext::*)(const Standard_Boolean)) &AIS_InteractiveContext::Select, "Stores and highlights the previous detected; Unhilights the previous picked.", py::arg("theToUpdateViewer"));
	...
	cls_AIS_InteractiveContext.def("SetToHilightSelected", (void (AIS_InteractiveContext::*)(const Standard_Boolean)) &AIS_InteractiveContext::SetToHilightSelected, "Specify whether selected object must be highlighted when mouse cursor is moved above it (in MoveTo method). By default this value is false and selected object is not highlighted in this case.", py::arg("toHilight"));
	cls_AIS_InteractiveContext.def("ToHilightSelected", (Standard_Boolean (AIS_InteractiveContext::*)() const ) &AIS_InteractiveContext::ToHilightSelected, "Return value specified whether selected object must be highlighted when mouse cursor is moved above it");
	...
	cls_AIS_InteractiveContext.def("SubIntensityOn", (void (AIS_InteractiveContext::*)(const Standard_Boolean)) &AIS_InteractiveContext::SubIntensityOn, "highlights/unhilights displayed objects which are displayed at neutral state with subintensity color. Available only for active local context. No effect if no local context.", py::arg("theToUpdateViewer"));
	...
	cls_AIS_Trihedron.def("HilightOwnerWithColor", (void (AIS_Trihedron::*)(const opencascade::handle<PrsMgr_PresentationManager3d> &, const opencascade::handle<Prs3d_Drawer> &, const opencascade::handle<SelectMgr_EntityOwner> &)) &AIS_Trihedron::HilightOwnerWithColor, "Method which highlight an owner belonging to this selectable object ( for fast presentation draw ).", py::arg("thePM"), py::arg("theStyle"), py::arg("theOwner"));
	...
	cls_AIS_LengthDimension.def("FirstPoint", (const gp_Pnt & (AIS_LengthDimension::*)() const ) &AIS_LengthDimension::FirstPoint, "Returns first attachment point.");
	cls_AIS_LengthDimension.def("SecondPoint", (const gp_Pnt & (AIS_LengthDimension::*)() const ) &AIS_LengthDimension::SecondPoint, "Returns second attachment point.");
	cls_AIS_LengthDimension.def("FirstShape", (const TopoDS_Shape & (AIS_LengthDimension::*)() const ) &AIS_LengthDimension::FirstShape, "Returns first attachment shape.");
	cls_AIS_LengthDimension.def("SecondShape", (const TopoDS_Shape & (AIS_LengthDimension::*)() const ) &AIS_LengthDimension::SecondShape, "Returns second attachment shape.");
	cls_AIS_LengthDimension.def("SetMeasuredGeometry", (void (AIS_LengthDimension::*)(const gp_Pnt &, const gp_Pnt &, const gp_Pln &)) &AIS_LengthDimension::SetMeasuredGeometry, "Measure distance between two points. The dimension will become invalid if the new distance between attachment points is less than Precision::Confusion().", py::arg("theFirstPoint"), py::arg("theSecondPoint"), py::arg("thePlane"));
	...
	*/
	...
	cls_AIS_Manipulator.def("Transform", (gp_Trsf (AIS_Manipulator::*)(const Standard_Integer, const Standard_Integer, const opencascade::handle<V3d_View> &)) &AIS_Manipulator::Transform, "Apply transformation made from mouse moving from start position (save on the first Transform() call and reset on DeactivateCurrentMode() call.) to the in/out mouse position (theX, theY)", py::arg("theX"), py::arg("theY"), py::arg("theView"));
	cls_AIS_Manipulator.def("ObjectTransformation", (Standard_Boolean (AIS_Manipulator::*)(const Standard_Integer, const Standard_Integer, const opencascade::handle<V3d_View> &, gp_Trsf &)) &AIS_Manipulator::ObjectTransformation, "Computes transformation of parent object according to the active mode and input motion vector. You can use this method to get object transformation according to current mode or use own algorithm to implement any other transformation for modes.", py::arg("theX"), py::arg("theY"), py::arg("theView"), py::arg("theTrsf"));
	...
	cls_AIS_Manipulator.def("HasActiveMode", (Standard_Boolean (AIS_Manipulator::*)() const ) &AIS_Manipulator::HasActiveMode, "Returns true if some part of manipulator is selected (transformation mode is active, and owning object can be rtansformated).");
	...
	py::class_<AIS_PointCloud, opencascade::handle<AIS_PointCloud>, AIS_InteractiveObject> cls_AIS_PointCloud(mod, "AIS_PointCloud", "Interactive object for set of points. The presentation supports two display modes: - Points. - Bounding box for highlighting. Presentation provides selection by bounding box. Selection and consequently highlighting can disabled by setting default selection mode to -1. There will be no way to select object from interactive view. Any calls to AIS_InteractiveContext::AddOrRemoveSelected should be also prohibited, to avoid programmatic highlighting (workaround is setting non-supported highlight mode, e.g. 100);");
	...
	py::class_<AIS_RadiusDimension, opencascade::handle<AIS_RadiusDimension>, AIS_Dimension> cls_AIS_RadiusDimension(mod, "AIS_RadiusDimension", "Radius dimension. Can be constructued: - On generic circle. - On generic circle with user-defined anchor point on that circle. - On generic shape containing geometry that can be measured by diameter dimension: circle wire, arc, circular face, etc. The anchor point is the location of left attachment point of dimension on the circle. It can be user-specified, or computed as middle point on the arc. The radius dimension always lies in the plane of the measured circle. The dimension is considered as invalid if the user-specified anchor point is not lying on the circle, if the radius of the circle is less than Precision::Confusion(). In case if the dimension is built on the arbitrary shape, it can be considered as invalid if the shape does not contain circle geometry.");
