...
PYBIND11_MODULE(Cocoa, mod) {
	...
	py::class_<NSAutoreleasePool, std::unique_ptr<NSAutoreleasePool, Deleter<NSAutoreleasePool>>> cls_NSAutoreleasePool(mod, "NSAutoreleasePool", "None");
	cls_NSAutoreleasePool.def(py::init<>());
	*/
	...
	py::class_<NSView, std::unique_ptr<NSView, Deleter<NSView>>> cls_NSView(mod, "NSView", "None");
	cls_NSView.def(py::init<>());
	*/

	/* FIXME
	// C:\Miniconda\envs\occt\Library\include\opencascade\Cocoa_Window.hxx
	py::class_<NSWindow, std::unique_ptr<NSWindow, Deleter<NSWindow>>> cls_NSWindow(mod, "NSWindow", "None");
	cls_NSWindow.def(py::init<>());
	*/
