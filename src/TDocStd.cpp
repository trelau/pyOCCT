...
PYBIND11_MODULE(TDocStd, mod) {
	...
	cls_TDocStd_Document.def("IsChanged", (Standard_Boolean (TDocStd_Document::*)() const ) &TDocStd_Document::IsChanged, "returns True if document differs from the state of last saving. this method have to be called only working in the transaction mode");
	...
	cls_TDocStd_Document.def("GetPath", (TCollection_ExtendedString (TDocStd_Document::*)() const ) &TDocStd_Document::GetPath, "returns the OS path of the file, in which one <me> is saved. Raise an exception if <me> is not saved.");
	...
	cls_TDocStd_XLink.def("BackupCopy", (opencascade::handle<TDF_Attribute> (TDocStd_XLink::*)() const ) &TDocStd_XLink::BackupCopy, "Returns a null handle. Raise always for ,it is nonsense to use this method.");
	...
	py::class_<TDocStd_Modified, opencascade::handle<TDocStd_Modified>, TDF_Attribute> cls_TDocStd_Modified(mod, "TDocStd_Modified", "Transient attribute which register modified labels. This attribute is attached to root label.");
